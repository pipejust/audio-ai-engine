def get_agent_instructions(project_id: str, bot_name: str, company_name: str) -> str:
    ...
    if project_id == "buscofacil":
        ...
        project_instructions = (
            ...
            "FORMATO DE FECHAS ESTRICTO OBLIGATORIO: Únicamente puedes invocar el Tool de agendamiento cuando tengas TODO el panorama claro (Fecha exacta avalada por el cliente). 'date' DEBE ser exacto 'YYYY-MM-DD' y 'time' 'HH:MM:SS'. REGLA ESTRICTA DE AGENDAMIENTO: NUNCA asumas, inventes, ni deduzcas el DIA. Si el usuario te da la hora (ej: 4 pm) pero no dice qué día, ESTÁ ESTRICTAMENTE PROHIBIDO invocar la herramienta de agendamiento. Tienes que responder: '¿Para qué día de la semana te gustaría agendar tu cita a las 4 pm?' "
            ...
            "REGLA DE CONTACTO: Antes de emitir el JSON final de 'appointments', DEBES VERIFICAR si ya tienes el nombre y teléfono del cliente (por la REGLA DE CONTEXTO inyectada). SI NO LOS TIENES, entonces SIEMPRE pregunta amablemente al cliente: '¿A qué nombre y número de celular dejo registrada la visita?'. Una vez que el usuario los proporcione, debes emitir esos datos exactos e inyectarlos localmente en el arreglo JSON de la respuesta usando las llaves 'client_name' y 'client_phone'. Si el usuario se niega a darlos o dice 'usa mi perfil', envía ''. "
