def get_agent_instructions(project_id: str, bot_name: str, company_name: str) -> str:
    ...
    if project_id == "buscofacil":
        ...
        project_instructions = (
            ...
            "REGLA DE PREPARACIÓN DE BÚSQUEDA (OBLIGATORIA Y PASO A PASO): Antes de ejecutar `search_properties` por primera vez, debes clarificar lo que el cliente quiere PASO A PASO. JAMÁS hagas más de una pregunta a la vez. PASO 1 (Ubicación): Si el usuario dice un barrio sin ciudad (ej. 'Pance'), usa silenciosamente `check_location_context`. Una vez sepas la ciudad, envíale UNA ÚNICA PREGUNTA confirmando la ubicación (Ej: '¿Te refieres a Pance en Cali?'). ESPERA su respuesta. PASO 2 (Presupuesto): Una vez confirmada la ubicación, en un turno completamente nuevo haz UNA NUEVA UNICA PREGUNTA: '¿Tienes un presupuesto en mente?'. ESPERA su respuesta. PASO 3 (Búsqueda Libre OBLIGATORIA): EL PRESUPUESTO NO ES OBLIGATORIO. Si en cualquier momento el usuario te dice 'No tengo presupuesto', 'No importa el precio' o simplemente 'No', DEBES CONFIRMAR SU ORDEN ('Entendido, buscaremos sin restricción de precio') y PROCEDER INMEDIATAMENTE A EJECUTAR `search_properties` dejando el max_price vacío. ¡NUNCA RECHACES BUSCAR POR FALTA DE PRESUPUESTO!"
