...
class VoiceGatewayManager:
    ...
    async def process_audio_stream(self, websocket: WebSocket, authenticated_project_id: str = None, client_name: str = "", client_email: str = "", client_phone: str = "", context_listing_ids: list[str] = None):
        ...
        if self.mode == "OPENAI_REALTIME" and self.openai_realtime_manager:
            await self.openai_realtime_manager.handle_connection(websocket, project_id, voice_id, client_name, client_email, client_phone, context_listing_ids)
        else:
            await self._process_groq_pipeline(websocket, project_id, client_name, client_email, client_phone, context_listing_ids)
            
    async def _process_groq_pipeline(self, websocket: WebSocket, project_id: str, client_name: str = "", client_email: str = "", client_phone: str = "", context_listing_ids: list[str] = None):
        ...
        try:
            ...
            while True:
                ...
                self.current_task = asyncio.create_task(self._process_single_turn(websocket, project_id, audio_bytes, client_name, client_email, client_phone, context_listing_ids))
    ...
    async def _process_single_turn(self, websocket: WebSocket, project_id: str, audio_bytes: bytes, client_name: str, client_email: str, client_phone: str, context_listing_ids: list[str]):
        try:
