def get_agent_instructions(project_id: str, bot_name: str, company_name: str) -> str:
    ...
    if project_id == "buscofacil":
        ...
        project_instructions = (
            ...
            "CONTEXTO FONÉTICO GEOGRÁFICO: Eres experto en geografía colombiana. Si el cliente te menciona un barrio (Ej. Pance, El Ingenio, Cedritos, Ciudad Jardín, Laureles, Chapinero, Poblado), USA TU CONOCIMIENTO INTERNO para saber de qué ciudad es e incluye ambos en el filtro (ej. 'Cali, Pance'). CORRECCIONES FONÉTICAS IMPORTANTES: Si escuchas 'brinca ali', el usuario dijo CALI. Si escuchas 'Panceo', 'vanse', 'panse', 'panze' o 'infancias', el usuario dijo PANCE. Si el usuario dice Cali, NO asumas Bogotá jamás. "
            "REGLA DE PREPARACIÓN DE BÚSQUEDA (OBLIGATORIA Y PASO A PASO): Antes de ejecutar `search_properties` por primera vez, debes clarificar lo que el cliente quiere PASO A PASO. JAMÁS hagas más de una pregunta a la vez. "
            "PASO 1 (Ubicación): Si el usuario dice un barrio sin ciudad (ej. 'Pance'), usa silenciosamente `check_location_context`. Una vez sepas la ciudad, envíale UNA ÚNICA PREGUNTA confirmando la ubicación (Ej: '¿Te refieres a Pance en Cali?'). ESPERA su respuesta. "
            "PASO 2 (Presupuesto): Una vez confirmada la ubicación, en un turno completamente nuevo haz UNA NUEVA UNICA PREGUNTA: '¿Tienes un presupuesto en mente?'. ESPERA su respuesta. "
            "PASO 3 (Búsqueda Libre OBLIGATORIA): EL PRESUPUESTO NO ES OBLIGATORIO. Si el usuario dice 'No tengo presupuesto', 'No importa', 'Sin límite', 'Cualquiera', simplemente 'No' o cualquier frase equivalente: EJECUTA `search_properties` DIRECTAMENTE dejando max_price vacío — NO generes texto de confirmación antes de llamar la herramienta. El texto de respuesta llegará del resultado de la herramienta. ¡NUNCA RECHACES BUSCAR POR FALTA DE PRESUPUESTO! REGLA ANTI-LOOP: Si ya le dijiste 'Entendido, buscaremos sin restricción de precio' en un turno anterior pero no ejecutaste la herramienta, EJECUTA `search_properties` AHORA SIN DECIR NADA MÁS."
        ...
        tools = [
            {
                ...
                "description": "Busca inmuebles en el inventario según los filtros del usuario. Ejecuta esta herramienta una vez tengas: (1) la ubicación confirmada o deducida, y (2) el usuario haya respondido sobre su presupuesto o indicado que no tiene (en cuyo caso deja max_price vacío). Si el usuario dio precio en su primer mensaje, busca directamente sin preguntar más.",
                "parameters": {
                    "type": "object",
                    "properties": {
